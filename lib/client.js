...
`
    ...
    // ===== 第一层：执行摘要 =====
    function SummaryView({ summary, onStory, onTree, onSelectFile, devMode }) {
    ...
    // ===== 第二层：执行故事线 =====
    function StoryView({ story, baseTime, onOpenEvent, devMode }) {
      ...
      return React.createElement('div', { className: 'dsvz-ov', onMouseDown: (e) => { if (e.target === e.currentTarget) onClose() } },
        React.createElement('div', { className: 'dsvz-box' },
          React.createElement('div', { className: 'dsvz-head' },
            ...
            React.createElement('span', { className: 'dsvz-pill', title: meta?.title ?? sessionId }, meta?.title ?? '未命名会话'),
          ...
          mode === 'story' && React.createElement(StoryView, { story, baseTime: meta?.startTime ?? summary?.startTime, devMode }),
