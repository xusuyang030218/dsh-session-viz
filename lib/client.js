...
`;
		...
		function HomeView({ closure, meta, turns, onJump, onOpenTree, onOpenSummary, onOpenMap }) {
			const [expandTurn, setExpandTurn] = react.default.useState(null);
			...
			function TurnRing({ ring }) {
				...
				const R = 64;
				const kids = ring.children || [];
				const stepSegs = kids.map((st) => ringArc(st, 50)).flat();
				...
				return react.default.createElement("div", {
					...
					onClick: (e) => {
						e.stopPropagation();
						setExpandTurn(expanded ? null : ring.turn);
					},
					...
					width: 160,
					height: 160
				}, outer.map((seg, i) => react.default.createElement("circle", {
					key: i,
					cx: 0,
					cy: 0,
					r: R,
					...
					strokeWidth: 6,
					strokeDasharray: dashOf(seg[2]) ?? void 0,
					strokeLinecap: "round"
				})), react.default.createElement("circle", {
					cx: 0,
					cy: 0,
					...
					strokeWidth: 7,
					strokeDasharray: dashOf(ring.status) ?? void 0,
					strokeLinecap: "round"
				}), react.default.createElement("circle", {
					cx: 0,
					cy: 0,
					...
					strokeWidth: 1
				}), stepSegs.map((seg, i) => react.default.createElement("path", {
					key: i,
					d: arcPath(seg[0], seg[1], 50),
					fill: "none",
					stroke: colorOf(seg[2]),
					strokeWidth: 4,
					strokeDasharray: dashOf(seg[2]) ?? void 0,
				...
				}, `T${ring.turn}`), react.default.createElement("text", {
					x: 0,
					y: 12,
					textAnchor: "middle",
					fontSize: 9.5,
					fill: "var(--dsw-alias-label-secondary,#8493ab)"
				}, `${kids.length} 步`)), react.default.createElement("div", { className: "dsvz-ring-meta" }, react.default.createElement("span", { className: "dsvz-ring-label" }, ring.label), react.default.createElement("span", { className: "dsvz-ring-status" }, ring.status === "closed" ? "✓ 闭合" : ring.status === "open" ? "◌ 进行中" : "✕ 失败"), ring.durationMs != null && react.default.createElement("span", { className: "dsvz-ring-dur" }, fmtDur(ring.durationMs))), expanded && react.default.createElement("div", { className: "dsvz-ring-steps" }, kids.map((st, i) => react.default.createElement("div", {
					key: i,
					className: "dsvz-ring-step",
					style: { borderLeftColor: colorOf(st.status) },
					onClick: (e) => {
						e.stopPropagation();
						onJump && onJump(st);
					},
				...
				}, st.status === "open" ? "◌" : ""), react.default.createElement("span", { className: "dsvz-ring-step-label" }, st.label), react.default.createElement("span", { className: "dsvz-ring-step-tools" }, st.children.filter((c) => c.kind === "tool").slice(0, 6).map((c, j) => react.default.createElement("span", {
					key: j,
					className: "dsvz-ring-tool" + (c.status === "error" ? " err" : c.status === "open" ? " open" : ""),
					title: `${c.label}${c.durationMs != null ? " · " + fmtDur(c.durationMs) : ""}${c.status === "error" ? " · 失败" : c.status === "open" ? " · 进行中" : ""}`
				}, c.label)), st.children.filter((c) => c.kind === "tool").length > 6 && react.default.createElement("span", { className: "dsvz-ring-more" }, `+${st.children.filter((c) => c.kind === "tool").length - 6}`)), st.durationMs != null && react.default.createElement("span", { className: "dsvz-ring-dur" }, fmtDur(st.durationMs))))));
			}
			return react.default.createElement("div", { className: "dsvz-scroll" }, react.default.createElement("div", { className: "dsvz-home" }, react.default.createElement("div", { className: "dsvz-home-hero" }, react.default.createElement("div", { className: "dsvz-home-title" }, "会话过程"), react.default.createElement("div", { className: "dsvz-home-sub" }, "把整个会话拆成一圈圈「环」：每一轮、每一步、每一次工具调用、每一次审批，都有开始与结束。闭合的环是已完成的工作，未闭合的环是正在进行或中断的工作。"), react.default.createElement("div", { className: "dsvz-home-legend" }, react.default.createElement("span", { className: "dsvz-legend-item" }, react.default.createElement("span", {
			...
			}, react.default.createElement("div", { className: "dsvz-home-card-icon" }, c.icon), react.default.createElement("div", { className: "dsvz-home-card-body" }, react.default.createElement("div", { className: "dsvz-home-card-label" }, c.label), react.default.createElement("div", { className: "dsvz-home-card-num" }, fmtNum(c.total))), react.default.createElement("div", { className: "dsvz-home-card-stats" }, react.default.createElement("span", { className: "dsvz-stat-ok" }, `✓ ${fmtNum(c.closed)}`), c.open > 0 && react.default.createElement("span", { className: "dsvz-stat-open" }, `◌ ${fmtNum(c.open)}`), c.error > 0 && react.default.createElement("span", { className: "dsvz-stat-err" }, `✕ ${fmtNum(c.error)}`))))), s.unclosed && s.unclosed.length > 0 && react.default.createElement("div", { className: "dsvz-home-unclosed" }, react.default.createElement("div", { className: "dsvz-home-unclosed-title" }, "◌ 进行中的工作（未闭合）"), react.default.createElement("div", { className: "dsvz-home-unclosed-list" }, s.unclosed.map((r, i) => react.default.createElement("div", {
				key: i,
			...
			}, `跳到第 ${r.turn} 轮${r.step != null ? "·" + r.step + " 步" : ""}`))))), react.default.createElement("div", { className: "dsvz-home-rings" }, (closure.rings || []).map((ring) => react.default.createElement(TurnRing, {
				key: ring.id,
				ring
		...
		function nearestAngle(sorted, byId, time) {
		...
		*/
		function loopLayout(snapshot) {
			...
			const angleById = /* @__PURE__ */ new Map();
			const turns = [];
			...
			for (const session of snapshot.sessions) {
				...
				for (const ring of turns) {
					...
					if (first !== void 0 && ringLast !== void 0 && session.createdAt >= first.node.time && session.createdAt <= ringLast.node.time) {
				...
				const startTime = nodes[0]?.time ?? session.createdAt;
			...
			return {
				turns,
				branches,
				maxRadius: turns.length > 0 ? turns.at(-1).radius + LOOP_RING_STEP : 94
			};
		}
		//#endregion
		...
		function arcPath(radius, angleFrom, angleTo, gap = .015) {
		...
		function LoopMap({ snapshot, selectedId, onSelect }) {
			...
			const max = layout.maxRadius + 54;
			const size = max * 2;
			const showTip = (node, event) => {
			...
			const nodeDot = (ringNode, radius, key) => {
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("svg", {
					...
					"aria-label": "会话执行闭环图",
					children: [
						layout.turns.map((ring) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)("circle", {
							r: ring.radius,
							fill: "rgba(255,255,255,0.02)",
							stroke: "rgba(120,150,190,0.14)",
							strokeWidth: 1
						}, `track-${ring.turn}`)),
						...
						layout.branches.map((branch, index) => {
							...
							return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("g", { children: [
								...
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)("circle", {
									...
									stroke: "#43d4d2",
									strokeWidth: 2.2,
									strokeDasharray: running ? "4 3" : void 0,
									className: running ? "seelogRunning" : void 0
								}),
						...
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)("text", {
							...
							fontWeight: 800,
							fill: "#e4eef8",
							children: snapshot.rootSessionId.slice(0, 10)
						}),
						/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("text", {
							textAnchor: "middle",
							y: 8,
							fontSize: 10.5,
							fill: "#91a7bc",
							children: [String(snapshot.sessions.reduce((total, session) => total + session.nodes.length, 0)), " 节点"]
						}),
						/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("text", {
							textAnchor: "middle",
							y: 22,
...
.seelogHeader{display:flex;gap:16px;justify-content:space-between;align-items:flex-start;border-bottom:1px solid var(--s-line);padding-bottom:15px}.seelogHeader h1{font-size:18px;margin:2px 0 3px;font-weight:650}.seelogEyebrow,.seelogMeta{color:var(--s-muted);margin:0}.seelogEyebrow{text-transform:uppercase;font-size:11px;letter-spacing:1.2px}.seelogActions{display:flex;gap:8px;flex-wrap:wrap}.seelogActions button,.seelogSnapshots button{border:1px solid #42647e;background:#1a2c3d;color:var(--s-text);border-radius:5px;padding:7px 10px;cursor:pointer;font:inherit}.seelogActions button:hover,.seelogSnapshots button:hover{border-color:var(--s-blue)}.seelogActions .seelogPrimary{background:#226bc0;border-color:#3e8be5}.seelogStats{display:flex;gap:20px;padding:13px 0;border-bottom:1px solid var(--s-line)}.seelogStats b{display:block;font-size:16px}.seelogStats span{color:var(--s-muted);font-size:11px}.seelogLayout{display:grid;grid-template-columns:minmax(0,1fr) 262px;gap:18px;padding-top:18px}.seelogMap{position:relative;min-height:450px;border:1px solid var(--s-line);background:#12202e;overflow:hidden}.seelogMapHeader{position:absolute;z-index:2;top:12px;left:15px;right:15px;display:flex;justify-content:space-between;color:var(--s-muted);pointer-events:none}.seelogCanvas{height:310px;border-bottom:1px solid var(--s-line)}.seelogLanes{padding:14px 15px 19px;overflow-x:auto}.seelogLane{display:grid;grid-template-columns:135px minmax(max-content,1fr);gap:12px;min-height:58px;border-top:1px solid rgba(66,100,126,.45);padding:10px 0}.seelogLane:first-child{border-top:0}.seelogLaneName{font-weight:600;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.seelogLaneName small{display:block;font-weight:400;color:var(--s-muted);margin-top:2px}.seelogEvents{display:flex;gap:7px;align-items:center}.seelogEvent{border:0;border-left:2px solid var(--s-blue);background:#1b2c3c;color:var(--s-text);border-radius:2px;padding:5px 7px;min-width:74px;max-width:142px;text-align:left;cursor:pointer;font:12px/1.25 inherit}.seelogEvent[data-kind="tool"]{border-color:var(--s-green)}.seelogEvent[data-kind="error"]{border-color:var(--s-red);background:#34242b}.seelogEvent[data-selected="true"]{outline:1px solid var(--s-blue);background:#243d54}.seelogEvent span{display:block;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.seelogEvent small{color:var(--s-muted);font-size:10px}.seelogSide{border-left:1px solid var(--s-line);min-height:450px;padding-left:18px}.seelogSide h2{font-size:13px;margin:1px 0 10px}.seelogEmpty,.seelogLoading{color:var(--s-muted);padding:18px 0}.seelogDetail{border-top:2px solid var(--s-blue);background:#142535;padding:12px}.seelogDetail p{margin:0 0 9px;color:var(--s-muted)}.seelogDetail b{color:var(--s-text)}.seelogSnapshots{border-top:1px solid var(--s-line);margin-top:18px;padding-top:14px}.seelogSnapshots h2{margin:0 0 9px;font-size:13px}.seelogSnapshots ul{list-style:none;margin:0;padding:0;display:grid;gap:6px}.seelogSnapshots button{width:100%;text-align:left}.seelogSnapshots button[aria-pressed="true"]{border-color:var(--s-blue)}.seelogNotice{color:#f5ca72;margin:12px 0 0}.seelogError{color:#ff9da3;margin:18px 0}@media(max-width:820px){.seelogRoot{padding:15px}.seelogHeader{display:block}.seelogActions{margin-top:12px}.seelogLayout{grid-template-columns:1fr}.seelogSide{border-left:0;border-top:1px solid var(--s-line);padding:15px 0 0;min-height:0}.seelogMap{min-height:390px}.seelogLane{grid-template-columns:104px minmax(max-content,1fr)}}`;
			...
			style.textContent += `
/* ===== 闭环轮环图（Loop Ring Map）===== */
.seelogRingWrap{position:relative;width:100%;height:100%;display:flex;align-items:center;justify-content:center;overflow:auto;padding:12px}
.seelogRing{display:block;width:auto;height:auto;max-width:100%;max-height:100%}
