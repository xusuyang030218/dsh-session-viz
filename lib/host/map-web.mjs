...
function projectEvents(sessionId, events, maximum) {
	...
	for (const event of tail) {
		...
		if (event.type === "assistant/message" && typeof surfaceOp === "object" && surfaceOp !== null && surfaceOp.op === "replace" && Array.isArray(sourceEventSeqs)) {
			const src = ((event.data ?? {}).message ?? {}).source ?? {};
			if (src.provider === "dsh-rewind" || src.model === "rewind-marker") {
				if (seq !== null) markerSeqs.add(seq);
				const target = Number(surfaceOp.start ?? sourceEventSeqs[0]);
	...
	for (const event of tail) {
		...
		switch (event.type) {
			...
			case "turn/end": if (event.data.reason.kind === "error") nodes.push(nodeFor(event, sessionId, {
				...
				title: "Turn failed",
				status: "error",
				turn: currentTurn ?? event.data.turn,
	...
	return {
		nodes,
		omittedEvents,
		rewindCount: markerSeqs.size,
		withdrawnEventCount: withdrawnSeqs.size
	};
}
...
async function resolveSessionId(ctx, sessionId) {
	let lastError;
	for (const candidate of sessionCandidates(sessionId)) try {
		await ctx.sessionQuery.traceSession(SessionId(candidate));
		return candidate;
	} catch (error) {
		lastError = error;
	}
...
function apply(ctx, config) {
	ctx.effect(() => ctx.webServer.register({
		...
		async handler(request, response) {
			...
			try {
				sendJson(response, 200, await snapshot(ctx, await resolveSessionId(ctx, sessionId), config));
			} catch (error) {
				sendJson(response, 404, {
					ok: false,
					code: error instanceof Error && error.message.includes("not found") ? "SESSION_LOG_NOT_FOUND" : "SESSION_READ_FAILED",
		...
		async handler(request, response) {
			...
			try {
				...
				const event = await ctx.sessionQuery.readEvent({
					sessionId: SessionId(resolvedId),
					seq: location.seq,
					before: 2,
					after: 2
				});
