...
function isRewindMarker(o) {
	...
	if (typeof sop !== "object" || sop === null || sop.op !== "replace") return false;
	if (!Array.isArray(o.sourceEventSeqs)) return false;
	const src = ((o.data ?? {}).message ?? {}).source ?? {};
	return src.provider === "dsh-rewind" || src.model === "rewind-marker";
}
/** 扫描事件序列，识别 dsh-rewind 回退标记并推导撤回区间（纯函数，无 I/O）。 */
function detectRewinds(objs) {
	const markers = [];
	const markerSeqs = /* @__PURE__ */ new Set();
	const noiseSeqs = /* @__PURE__ */ new Set();
	const seqAt = /* @__PURE__ */ new Map();
	for (let i = 0; i < objs.length; i++) {
		const o = objs[i];
		if (!o) continue;
		const seq = o.seq ?? o.seq0;
		if (typeof seq === "number") seqAt.set(seq, o);
		if (!isRewindMarker(o)) continue;
		const d = o.data ?? {};
		const sop = o.surfaceOp;
		const targetSeq = Number(sop.start ?? o.sourceEventSeqs[0]);
		const sourceEventSeqs = o.sourceEventSeqs.map((s) => Number(s)).filter((s) => Number.isSafeInteger(s));
		const turn = d.turn ?? null;
		const step = d.step ?? null;
		let ghostStartSeq = null;
		let ghostEndSeq = null;
		for (let j = i - 1; j >= 0; j--) {
			const prev = objs[j];
			if (!prev) continue;
			if (prev.type !== "step/start") break;
			const pd = prev.data ?? {};
			if (pd.turn === turn && pd.step === step) {
				ghostStartSeq = prev.seq ?? null;
				break;
			}
		}
		for (let j = i + 1; j < objs.length; j++) {
			...
			const nd = next.data ?? {};
			if (nd.turn === turn && nd.step === step) {
				ghostEndSeq = next.seq ?? null;
				break;
			}
		}
		const markerSeq = o.seq ?? null;
	...
	const withdrawnSeqs = /* @__PURE__ */ new Set();
	for (const marker of markers) {
		const lo = marker.targetSeq;
		const hi = marker.markerSeq;
		if (lo === null || hi === null) continue;
		let count = 0;
		for (const seq of seqAt.keys()) if (seq >= lo && seq < hi) {
			withdrawnSeqs.add(seq);
			count++;
		}
...
function rewindOptsOf(objs) {
	const { markers, markerSeqs, noiseSeqs, withdrawnSeqs } = detectRewinds(objs);
	return {
		withdrawn: withdrawnSeqs,
		noiseSeqs,
		markerSeqs,
		markers
	};
}
function ringLabel(kind, d, fallback) {
...
function ringDuration(openTime, closeTime) {
	if (openTime == null || closeTime == null) return void 0;
...
*/
function buildClosure(objs, opts) {
	const rw = opts ?? rewindOptsOf(objs);
	const { withdrawn, noiseSeqs } = rw;
	const turns = [];
	const stack = [];
	const orphans = [];
	let curStepRing = null;
	let curTurnRing = null;
	const toolCalls = /* @__PURE__ */ new Map();
	const approvals = [];
	function pushToParent(ring) {
	...
	for (let i = 0; i < objs.length; i++) {
		...
		if (t === "turn/end") {
			...
			if (curTurnRing) {
				...
				if (reason.kind === "error") curTurnRing.detail = String(reason.error?.code ?? "error");
			}
			stack.pop();
			const top = stack.length ? stack[stack.length - 1] : void 0;
			curTurnRing = top !== void 0 && top.kind === "turn" ? top : null;
			curStepRing = top !== void 0 && top.kind === "step" ? top : null;
			continue;
		}
		if (t === "step/start") {
			const ring = {
		...
		if (t === "tool/result") {
			...
			if (ring) {
				...
				const hasError = d.error !== void 0;
				ring.status = hasError ? "error" : "closed";
				if (hasError) ring.detail = String(d.error?.code ?? "error");
			}
			continue;
		}
		if (t === "approval/asked") {
...
const TOOL_ZH = {
	...
	sandbox_logs: "沙盒日志",
	sandbox_build: "沙盒构建",
	code_workbench: "代码工作台",
	import_document: "导入文档",
	recommend_plugins: "插件推荐",
	search_plugins: "插件搜索",
	rank_plugins: "插件榜",
	trend_plugins: "插件趋势",
	sync_registry: "同步插件数据"
};
...
function buildTree(lines, objs, opts) {
...
function buildSummary(lines, objs, meta, typeCounts, opts) {
	const rw = opts ?? rewindOptsOf(objs);
	const summary = {
	...
	const revertedTurns = /* @__PURE__ */ new Set();
	{
		let curTurnNum = null;
		for (const o of objs) {
			if (!o) continue;
			...
			const d = o.data ?? {};
			const seq = o.seq ?? o.seq0;
			if (t === "turn/start") curTurnNum = d.turn;
			else if (t === "turn/end") curTurnNum = null;
			else if (t === "user/message" && typeof seq === "number" && rw.withdrawn.has(seq) && curTurnNum !== null) revertedTurns.add(curTurnNum);
		}
	}
	let curTurnNum = null;
	for (const o of objs) {
		...
		if (t === "tool/result") {
			...
			const path = metaObj.path ?? (tc ? pathFromArgs(JSON.stringify(tc.args)) : null);
			if (path && (name === "write" || name === "edit")) {
				...
				const change = {
					...
					error: isError,
					content: name === "write" ? content : void 0,
					oldString: name === "edit" ? oldString : void 0,
					newString: name === "edit" ? newString : void 0,
					preview: name === "edit" ? newString?.slice(0, 120) : content?.slice(0, 120)
				};
				if (isWithdrawn) {
					change.withdrawn = true;
					summary.rewoundFiles.push(change);
				} else if (curTurnNum === null || !revertedTurns.has(curTurnNum)) summary.files.push(change);
			}
			if (src) {
		...
		} else if (t === "request/context" && !summary.model && !isWithdrawn) summary.model = modelLabel(d.model);
		else if (t === "approval/asked") {
			if (!isWithdrawn) pendingApproval.set(d.id, {
				toolName: d.toolName,
				time: o.time ?? null
			});
	...
	{
		let curTurnNum2 = null;
		for (const o of objs) {
			if (!o) continue;
			const t = o.type ?? "?";
			const data = o.data ?? {};
			if (t === "turn/start") {
				curTurnNum2 = data.turn;
				continue;
			}
			if (t === "turn/end") {
				curTurnNum2 = null;
...
function targetPreviewOf(objs, targetSeq) {
	for (const o of objs) {
		if (!o || o.type !== "user/message") continue;
		if ((o.seq ?? o.seq0) !== targetSeq) continue;
		const joined = ((o.data ?? {}).content ?? []).map((p) => p.text ?? `[${p.type}]`).filter(Boolean).join(" ").trim();
...
function buildStory(lines, objs, opts) {
	const rw = opts ?? rewindOptsOf(objs);
	const turns = [];
	let cur = null;
	let stepBuf = null;
	/** 在故事线上插入一条「↶ 回退」节点（放在标记所在位置/所在轮末尾）。 */
	function pushRewindNode(marker) {
		if (!marker) return;
	...
	for (const o of objs) {
		...
		if (isWithdrawn) {
			const time = o.time ?? null;
			const turnNum = d.turn ?? cur?.turn ?? 0;
			const stepNum = d.step ?? stepBuf?.step ?? 0;
			if (t === "user/message") pushWithdrawn({
				kind: "user",
				time,
			...
			else if (t === "reasoning-chunks" || t === "text-chunks") pushWithdrawn({
				kind: "reasoning",
				time,
				text: (d.texts ?? []).join(""),
				human: "推理 / 输出内容（已回退）",
			...
			} else if (t === "approval/asked") pushWithdrawn({
				kind: "approval",
				time,
				id: d.id,
				...
				if (target) {
					target.outcome = d.outcome;
					target.outcomeHuman = d.outcome === "allowed-once" ? "✅ 已批准（一次）" : d.outcome === "allowed-always" ? "✅ 已批准（始终）" : d.outcome === "denied" ? "❌ 已拒绝" : d.outcome ?? "?";
				}
			} else if (t === "assistant/message") pushWithdrawn({
				kind: "assistant",
				time,
				text: ((d.message ?? {}).content ?? []).filter((p) => p.type === "text").map((p) => p.text).filter(Boolean).join(" "),
				human: "模型回复（已回退）",
				turn: turnNum,
				step: stepNum
			});
...
function summarizeType(o) {
	...
	switch (t) {
		...
		case "reasoning-chunks":
		case "text-chunks": return (d.texts ?? []).join("").slice(0, 120) || "—";
		case "tool-call-chunks": return `${d.name ?? "工具"} 参数分片（${(d.args ?? []).length} 段）`;
...
function eventLogText(session) {
	return [{
		type: "session",
		time: session.session.createdAt,
		createdAt: session.session.createdAt,
		cwd: session.session.cwd ?? null,
		data: { cwd: session.session.cwd ?? null }
...
async function getCached(sessionsPath, sessionId, sessionQuery) {
	const hit = CACHE.get(sessionId);
	if (hit && Date.now() - hit.at < CACHE_TTL_MS) return hit;
	if (!sessionsPath && sessionQuery) {
		let lastError;
		for (const candidate of sessionCandidates(sessionId)) try {
			const text = eventLogText(await sessionQuery.readSession(SessionId(candidate)));
			const parsed = parseLogText(text);
			parsed.meta.sizeBytes = Buffer.byteLength(text, "utf8");
			const entry = {
				parsed,
				text,
				path: `dsh-session-query:${candidate}`,
				at: Date.now()
			};
		...
		throw lastError instanceof Error ? lastError : /* @__PURE__ */ new Error(`session log not found: ${sessionId}`);
	}
...
async function listSessions(sessionsPath, sessionQuery) {
	if (!sessionsPath && sessionQuery) {
		const jobs = (await sessionQuery.listSessions()).map(async (record) => {
			const id = String(record.header.id);
			try {
				const entry = await getCached(null, id, sessionQuery);
				return {
					id,
					dirEncoded: "dsh-session-query",
					cwd: entry.parsed.meta.cwd ?? null,
					createdAt: entry.parsed.meta.createdAt ?? null,
					title: entry.parsed.meta.title ?? null,
			...
			try {
				...
				if (pathname === "/dsh-session-viz/api/sessions" && (req.method === "GET" || req.method === "HEAD")) {
					if (!sessionsPath && !sessionQuery) {
						json(res, 400, {
							ok: false,
							error: "当前 Profile 未提供本地日志或 sessionQuery"
						});
						return;
					}
					if (Date.now() - listCache.at > 1e4) listCache = {
						at: Date.now(),
						sessions: await listSessions(sessionsPath, sessionQuery)
					};
				...
				if (pathname === "/dsh-session-viz/api/log" && (req.method === "GET" || req.method === "HEAD")) {
					if (!sessionsPath && !sessionQuery || !q.sessionId) {
						json(res, 400, {
							ok: false,
							error: "sessionId 缺失或当前 Profile 不支持会话读取"
						});
						return;
					}
				...
				if (pathname === "/dsh-session-viz/api/story" && (req.method === "GET" || req.method === "HEAD")) {
					...
					const { lines, objs } = narrativeInput(await getCached(sessionsPath, q.sessionId, sessionQuery));
					json(res, 200, {
						ok: true,
						story: buildStory(lines, objs, rewindOptsOf(objs))
					});
					return;
				}
				...
				json(res, missing ? 404 : 500, {
					ok: false,
					code: missing ? "SESSION_LOG_NOT_FOUND" : "SESSION_READ_FAILED",
					error: message,
					hint: missing ? "该会话可能来自 Agent Anywhere 或尚未持久化；已切换到 sessionQuery 时请确认当前 Profile 支持它。" : void 0
				});
			}
		}
